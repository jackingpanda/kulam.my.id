document.addEventListener('DOMContentLoaded', () => {
    ...
    setInterval(() => {
        ...
        if(mainTitle) {
            ...
            mainTitle.style.fontWeight = randomWeight;
        }
    }, 100);
    ...
    const pointers = [{ id: -1, x: 0, y: 0, dx: 0, dy: 0, down: false, moved: false, color: [30, 0, 300] }];
    let lastTime = Date.now();
    
    // Shaders
    const baseVertexShaderSource = `precision highp float; precision mediump sampler2D; attribute vec2 aPosition; varying vec2 vUv; varying vec2 vL; varying vec2 vR; varying vec2 vT; varying vec2 vB; uniform vec2 texelSize; void main () { vUv = aPosition * 0.5 + 0.5; vL = vUv - vec2(texelSize.x, 0.0); vR = vUv + vec2(texelSize.x, 0.0); vT = vUv + vec2(0.0, texelSize.y); vB = vUv - vec2(0.0, texelSize.y); gl_Position = vec4(aPosition, 0.0, 1.0); }`;
    const clearShaderSource = `precision highp float; precision mediump sampler2D; varying vec2 vUv; uniform sampler2D uTexture; uniform float value; void main () { gl_FragColor = value * texture2D(uTexture, vUv); }`;
    const displayShaderSource = `precision highp float; precision mediump sampler2D; varying vec2 vUv; uniform sampler2D uTexture; void main () { gl_FragColor = texture2D(uTexture, vUv); }`;
    ...
    function getWebGLContext(canvas) {
        ...
        return { gl, ext: { internalFormat, internalFormatRG, formatRG, texType }, support_linear_float: !!support_linear_float };
    }

    const { ext, support_linear_float } = getWebGLContext(canvas);
    ...
    const clearProgram = new GLProgram(baseVertexShader, compileShader(gl.FRAGMENT_SHADER, clearShaderSource));
    const displayProgram = new GLProgram(baseVertexShader, compileShader(gl.FRAGMENT_SHADER, displayShaderSource));
    const splatProgram = new GLProgram(baseVertexShader, compileShader(gl.FRAGMENT_SHADER, splatShaderSource));
    const advectionProgram = new GLProgram(baseVertexShader, compileShader(gl.FRAGMENT_SHADER, advectionShaderSource));
    const divergenceProgram = new GLProgram(baseVertexShader, compileShader(gl.FRAGMENT_SHADER, divergenceShaderSource));
    const curlProgram = new GLProgram(baseVertexShader, compileShader(gl.FRAGMENT_SHADER, curlShaderSource));
    const vorticityProgram = new GLProgram(baseVertexShader, compileShader(gl.FRAGMENT_SHADER, vorticityShaderSource));
    const pressureProgram = new GLProgram(baseVertexShader, compileShader(gl.FRAGMENT_SHADER, pressureShaderSource));
    const gradienSubtractProgram = new GLProgram(baseVertexShader, compileShader(gl.FRAGMENT_SHADER, gradientSubtractShaderSource));
    ...
    function initFramebuffers() {
        ...
        density = createDoubleFBO(0, textureWidth, textureHeight, ext.internalFormat, gl.RGBA, texType, linear);
        velocity = createDoubleFBO(2, textureWidth, textureHeight, ext.internalFormatRG, ext.formatRG, texType, linear);
        divergence = createFBO(4, textureWidth, textureHeight, ext.internalFormatRG, ext.formatRG, texType, gl.NEAREST);
        curl = createFBO(5, textureWidth, textureHeight, ext.internalFormatRG, ext.formatRG, texType, gl.NEAREST);
        pressure = createDoubleFBO(6, textureWidth, textureHeight, ext.internalFormatRG, ext.formatRG, texType, gl.NEAREST);
    }
